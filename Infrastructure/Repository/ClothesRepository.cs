...
namespace Infrastructure.Repository
{
    public class ClothesRepository : IClothesRepository
    {
        ...
        public List<Clothes> GetList(string? key, int? pageSize, int? page)
        {
            using (var db = new MyDbContext())
            {
                var rs = db.Clothes.Include(e => e.TypeClothes).Include(e => e.DetailInvoices).Include(e => e.DetailInvoiceLaundries).ToList();
